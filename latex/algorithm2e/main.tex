...
$C \gets \emptyset$\;
\For(\tcc*[f]{for-comment}){$i \gets 1$ \textbf{to} $r$}{
  \While{$n \geq c_i$} {
    $C \gets C \cup \{c_i\}$\;
    $n \gets n - c_i$\;
  }
...
$location \gets 0$\;
$i \gets 2$\;
\While(\tcc*[f]{while-comment}){$i \leq n \land location = 0$}{
  $j \gets 1$\;
